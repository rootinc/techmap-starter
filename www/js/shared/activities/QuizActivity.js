...
*/
var QuizActivity = Activity.extend({
  init:function(properties)
  {
    ...
    */
    ...
    /**
    css class that 'corrects' the choice
    @property correctClass
    @type String
    @protected
    */
    this.correctClass = properties.correctClass || "correct";

    /**
    css class that 'incorrects' the choice
    @property incorrectClass
    @type String
    @protected
    */
    this.incorrectClass = properties.incorrectClass || "incorrect";
    ...
    [
      {
        ...
        text:"selector",
        answers:index || array of indexes,
        ?choices:{
    ...
    */
    ...
    /**
    flag for triggering autocorrect functions
    @property autoCorrect
    @type Number
    @protected
    */
    this.autoCorrect = properties.autoCorrect === undefined ? false : properties.autoCorrect;
    ...
    for (var i=0;i<this.qaList.length;i++)
    {
      ...
      switch (interactionType)
      {
        ...
        case "drag":
          ...
          //for each drop
          obj.$drops.each(function(index,el){
            var $el = $(el);
  ...
  */
    ...
    this.currentInteraction = new DragInteraction({
      ...
      events:{
        startDrag:function(e){
          that.page.module.canSwipe = false; //stop the cheating swipe for now
          that.hideAllResponses();
        },
        stopDrag:function(e){
          that.page.module.canSwipe = true; //stop the cheating swipe for now

          var $el = $(this);
          if ($el.data('drop'))
          {
            $el.addClass(that.selectedClass);
          }
          else
          {
            $el.removeClass(that.selectedClass);
          }

          if (that.currentInteraction.dropsAllDragged()) //if all drops have a drag
          {
            that.startFlow(e, $el, 2); //start the flow
          }
        }
  ...
  /**
  disables the current interaction
  @method disableCurrentInteraction
  @protected
  */
  disableCurrentInteraction:function()
  {
    if (this.currentInteraction)
    {
      this.currentInteraction.disable();
    }
  },
  ...
  */
  submit:function(e, $el)
  {
    ...
    if (this.checkAnswers()) //if they are correct
    {
      ...
      //puts the correct/incorrect classes on
      this.autoCorrectQuestion();
      this.disableCurrentInteraction(); //and disable the interaction
    }
    else //else they are stupid
    {
      ...
      if (this.attemptIndex < this.attemptMax)
      {
        if (this.autoCorrect)
        {
          this.addFeedback(); //show the incorrect questions
        }
        
        this.showTryAgainResponse(); //show try again
        this.bindSubmitButton(); //bind submit button again, because, it gets unbound
  ...
  */
  ...
  {    
    ...
    $drops.each(function(index, el){
      var $drop = $(el);
      var $drag = $drop.data('drag');

      //basically, if the drag is in the wrong spot (index)
      if ($drop.attr('data-correct') !== ""+$drags.index($drag))
      {
        isCorrect = false; //then we are wrong
        return false; //break out
      }
  ...
  /**
  corrects the questin based on the interaction type
  @method autoCorrectQuestion
  @protected
  */
  autoCorrectQuestion:function()
  {
    var interactionType = this.getInteractionType(); //get the interaction type
    ...
    switch (interactionType)
    {
      case "click":
        ...
        this.autoCorrectQuestionClick($clicks); //auto correct
        break;

      case "drag":
        var obj = this.getDragsAndDrops(); //get the drag object

        this.autoCorrectQuestionDrag(obj.$drags,obj.$drops); //auto correct
        break;
  ...
  /**
  corrects the question if it is a click type
  @method autoCorrectQuestionClick
  @param {jQuery} $clicks jquery clicks to check
  @protected
  */
  autoCorrectQuestionClick:function($clicks)
  {
    $clicks.filter('[data-correct]').addClass(this.correctClass); //add correct class for correct attributes
    $clicks.not('[data-correct]').addClass(this.incorrectClass); //add incorrect class for incorrect attributes
  },


  /**
  corrects the question if it is a drag
  @method autoCorrectQuestionDrag
  @param {jQuery} $drags jquery drags to check
  @param {jQuery} $drops jquery drops to check
  @protected
  */
  autoCorrectQuestionDrag:function($drags,$drops)
  {
    var that = this;
    ...
    $drops.each(function(index, el){
      var $drop = $(el);
      var $checkDrag = $drop.data('drag');

      //basically, if the drag is in the correct spot (index)
      if ($drop.attr('data-correct') === ""+$drags.index($checkDrag))
      {
        $checkDrag.addClass(that.correctClass);
      }
      else
      {
        $checkDrag.addClass(that.incorrectClass);
      }

      var $drag = $($drags[$drop.attr('data-correct')]); //get the 'correct' drag based on the data-correct index

      //since setPosition is damaging -- and we are still looping through each drop, we "hackishly" do this on a timeout
      (function($drag,$drop){
        setTimeout(function(){
          that.currentInteraction.setPosition($drag,$drop); //set the drag on the drop
        });
      })($drag,$drop);
    });
  },
  ...
  */
  ...
  /**
  add the selections based on the interaction type
  @method addFeedback
  @protected
  */
  addFeedback:function()
  {
    ...
    {
      case "click":
        this.addFeedbackClick(this.getClicks()); //remove selections based on clicks
      ...
      case "drag":
        ...
        this.addFeedbackDrag(obj.$drags,obj.$drops); //remove selections based on drags
  ...
  /**
  adds the selections for click type interactions
  @method addFeedbackClick
  @param {jQuery} $clicks the clicks to remove
  @protected
  */
  addFeedbackClick:function($clicks)
  {
    var that = this;
    ...
    $clicks.each(function(index, el){
      ...
      if ($el.hasClass(that.selectedClass) && $el.attr("data-correct") === undefined)
      {
        $el.addClass(that.incorrectClass);
      }
    });
  },

  /**
  adds the selections for drag type interactions
  @method addFeedbackDrag
  @param {jQuery} $drags jquery drags to be dragged
  @param {jQuery} $drops jquery drops to be dropped
  @protected
  */
  addFeedbackDrag:function($drags,$drops)
  {
    //todo implement
  },
  ...
  */
  removeFeedback:function()
  {
    ...
    {
      case "click":
        this.removeFeedbackClick(this.getClicks()); //remove selections based on clicks
      ...
      case "drag":
        ...
        this.removeFeedbackDrag(obj.$drags,obj.$drops); //remove selections based on drags
  ...
  @method removeFeedbackClick
  @param {jQuery} $clicks the clicks to remove
  @protected
  */
  removeFeedbackClick:function($clicks)
  {
    //remove selected class on the clicks
    $clicks.removeClass(this.selectedClass);
    $clicks.removeClass(this.correctClass);
    $clicks.removeClass(this.incorrectClass);
  },

  /**
  removes the selections for drag type interactions
  @method removeFeedbackDrag
  @param {jQuery} $drags jquery drags to be dragged
  @param {jQuery} $drops jquery drops to be dropped
  @protected
  */
  removeFeedbackDrag:function($drags,$drops)
  {
    this.currentInteraction.enable();

    //resets the positions
    this.currentInteraction.resetPositions();

    $drags.removeClass(this.selectedClass);
    $drags.removeClass(this.correctClass);
    $drags.removeClass(this.incorrectClass);
  },

  /**
  resets the selections for multiple tried questions
  ...
  */
  resetSelections:function()
  {
    ...
    {
      case "click":
        this.resetSelectionsClick(this.getClicks()); //reset selections based on clicks
      ...
      case "drag":
        ...
        this.resetSelectionsDrag(obj.$drags,obj.$drops); //reset selections based on drags
  ...
  /**
  removes the selections for click type interactions
  @method resetSelectionsClick
  @param {jQuery} $clicks the clicks to remove
  @protected
  */
  resetSelectionsClick:function($clicks)
  {
    //remove selected class on the clicks
    $clicks.removeClass(this.selectedClass);
  },
  ...
  /**
  removes the selections for drag type interactions
  @method resetSelectionsDrag
  @param {jQuery} $drags jquery drags to be dragged
  @param {jQuery} $drops jquery drops to be dropped
  @protected
  */
  resetSelectionsDrag:function($drags,$drops)
  {
    var that = this;
    ...
    $drops.each(function(index, el){
      ...
      if ($drop.attr('data-correct') !== ""+$drags.index($drag))
      {
        that.currentInteraction.resetPosition($drag);
        $drag.removeClass(that.selectedClass);
      }
      else
      {
        $drag.addClass(that.correctClass);
      }
    });
  },
  ...
  */
  destroy:function()
  {
    ...
    if (this.currentInteraction) //if we are on the last question, we've already done this
    {
      this.removeFeedback(); //remove the selections
    }
